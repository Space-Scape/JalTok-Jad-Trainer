...
func _on_RangedHitBox_area_entered(area):
	...
	if Globals.tick == 1:
		if Globals.prayingMage and area.get_name() == "BoulderR":
			...
			if Globals.ate:
				...
				hpOrb.value -= num
				numContainer.bbcode_text = "[center]" + str(num).pad_decimals(0) + "[/center]"
		if Globals.prayingRange and area.get_name() == "BoulderR":
			hitSplat.texture = load("res://Sprites/hitsplatBlue.png")
			hitSplat.show()
			Globals.hitDmg = false
			numContainer.bbcode_text = ""
		if Globals.prayingMelee and area.get_name() == "BoulderR":
			hitSplat.texture = load("res://Sprites/hitsplatRed.png")
			hitSplat.show()
			numContainer.bbcode_text = "[center]" + str(num).pad_decimals(0) + "[/center]"
			Globals.hitDmg = true
			if Globals.ate:
				...
				hpOrb.value -= num
				numContainer.bbcode_text = "[center]" + str(num).pad_decimals(0) + "[/center]"
		if Globals.prayOff and area.get_name() == "BoulderR":
			hitSplat.texture = load("res://Sprites/hitsplatRed.png")
			hitSplat.show()
			numContainer.bbcode_text = "[center]" + str(num).pad_decimals(0) + "[/center]"
			Globals.hitDmg = true
